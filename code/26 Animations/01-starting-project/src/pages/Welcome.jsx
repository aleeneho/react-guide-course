...
export default function WelcomePage() {
  ...
  const yCity = useTransform(scrollY, [0, 200], [0, -100]); // 0 to 200px, y 0 to -100px
  const opacityCity = useTransform(scrollY, [0, 200, 300, 500], [1, 0.5, 0.5, 0]); // 0 to 200px, opacity 1 to 0.5
  const yHero = useTransform(scrollY, [0, 200], [0, -150]); // 0 to 200px, y 0 to -150px
  const opacityHero = useTransform(scrollY, [0, 300, 500], [1, 1, 0]); // 0 to 300px, opacity 1 to 0
  const yText = useTransform(scrollY, [0,200, 300, 500], [0, 50, 50, 300]); // 0 to 200px, y 0 to 50px
  const scaleText = useTransform(scrollY, [0, 300], [1, 1.5]); // 0 to 300px, scale 1 to 1.5
